...
def warp_img(img: torch.Tensor, scr_pts: torch.Tensor, dst_pts: torch.Tensor, size=(128,128))->torch.Tensor:
    ...
    Args:
        img (Tensor): image to warp
        scr_pts (Tensor): source points
        dst_pts (Tensor): destination points
        size (tuple, optional): size. Defaults to (128,128).
...
class MultiModalFAN(nn.Module):
    def __init__(self, num_modules=1, n_points=66, in_channels=3):
        ...
        self.modal1 = ModalityLayer(in_channels)
        self.modal2 = ModalityLayer(in_channels)
